...
class BarcodeService {
  ...
  private async getSuggestionsForBarcode(barcode: string, pharmacyId?: string): Promise<Medicine[]> {
    try {
      // Try multiple search strategies for better medicine recognition
      const suggestions: Medicine[] = [];
      
      // Strategy 1: Partial barcode matches (first 8 digits)
      if (barcode.length >= 8) {
        ...
        if (pharmacyId) {
          partialQuery = partialQuery.eq('pharmacy_id', pharmacyId);
        }
      ...
      // Strategy 2: Similar barcode patterns (last 6 digits)
      if (barcode.length >= 6) {
        let similarQuery = supabase
          .from('medicines')
          .select('*')
          .ilike('barcode_number', `%${barcode.slice(-6)}%`)
          .eq('is_active', true)
          .limit(3);

        if (pharmacyId) {
          similarQuery = similarQuery.eq('pharmacy_id', pharmacyId);
        }

        const { data: similarMatches } = await similarQuery;
        if (similarMatches) {
          suggestions.push(...similarMatches.filter(med => 
            !suggestions.some(s => s.id === med.id)
          ));
        }
      }

      // Strategy 3: Recently scanned medicines (if we have scan logs)
      if (suggestions.length < 5) {
        ...
        if (recentMedicines) {
          suggestions.push(...recentMedicines.filter(med => 
            !suggestions.some(s => s.id === med.id)
          ));
        }
      }

      // Strategy 4: Popular medicines (most frequently sold)
      if (suggestions.length < 5) {
        ...
        if (pharmacyId) {
          popularQuery = popularQuery.eq('pharmacy_id', pharmacyId);
        }

        const { data: popularMedicines } = await popularQuery;
        if (popularMedicines) {
          suggestions.push(...popularMedicines.filter(med => 
            !suggestions.some(s => s.id === med.id)
          ));
        }
      }
